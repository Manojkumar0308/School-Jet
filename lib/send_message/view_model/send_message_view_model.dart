...
It updates the _registrationWise property and turns off other modes.
 After setting the mode, it notifies listeners.*/
  ...
  Future<void> registrationNoWiseApi(
      ...
      BuildContext context) async {
    final pref = await SharedPreferences.getInstance();
    // pref.setInt('notificationToken', 0);
    final baseurl = pref.getString('apiurl');
    ...
    final Map<String, dynamic> body = {
      ...
      "regnos": regNo,
      "sessionid": sessionId,
      "softwarename": "Notification"
    };
    print('body of regNoWiseApi $body');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        print('regWiseData response is $regWiseData');
        if (regWiseData['resultcode'] == 0) {
          ...
          final msgtoken = int.parse(regWiseData['token']);
          print('token is:');
          print(msgtoken);
          print(msgtoken.runtimeType);
          pref.setInt('notificationToken',
              msgtoken); // Save msgtoken to SharedPreferences
          globalnotificationToken = pref.getInt('notificationToken');
          print('msgid is $globalnotificationToken');

          // msgid(regWiseData['token']);
          notifyListeners();
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (result.containsKey("stm")) {
          ...
          // Check if regNumber is null and initialize it if necessary
          if (regNumber == null) {
            regNumber = [];
          }
