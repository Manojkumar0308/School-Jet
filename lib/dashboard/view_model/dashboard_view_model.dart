...
//method for saving data for selected student.
  ...
  Future<void> punchingData() async {
    final pref = await SharedPreferences.getInstance();
    ...
    try {
      if (baseurl != null) {
        final url = '$baseurl${hostService.getPunchDetailUrl}$schoolId';
        print('punchDetailUrl is--->$url');
        final response = await http.get(Uri.parse(url));
        if (response.statusCode == 200) {
          final result = jsonDecode(response.body);
          print('response of punchDetail is $result');
          Fluttertoast.showToast(msg: 'Something went wrong');
        }
      } else {
        print('baseurl is null');
      }
    } catch (e) {
      print(e.toString());
    }
